...
static server_cc::ConfigVar<std::string>::ptr g_module_path
...
bool Module::handleRequest(server_cc::Message::ptr req
                           ,server_cc::Message::ptr rsp
                           ,server_cc::Stream::ptr stream) {
    SERVER_CC_LOG_DEBUG(g_logger) << "handleRequest req=" << req->toString()
            << " rsp=" << rsp->toString() << " stream=" << stream;
    return true;
}

bool Module::handleNotify(server_cc::Message::ptr notify
                          ,server_cc::Stream::ptr stream) {
    SERVER_CC_LOG_DEBUG(g_logger) << "handleNotify nty=" << notify->toString()
            << " stream=" << stream;
    return true;
}
...
bool Module::onConnect(server_cc::Stream::ptr stream) {
    return true;
}

bool Module::onDisconnect(server_cc::Stream::ptr stream) {
    return true;
}
    ...
    for(auto& i : svrs) {
        ...
        for(auto& s : socks) {
            ...
            if(str.find("0.0.0.0") == 0) {
                ip_and_port = server_cc::GetIPv4() + ":" + std::to_string(addr->getPort());
            } else {
...
bool RockModule::handleRequest(server_cc::Message::ptr req
                               ,server_cc::Message::ptr rsp
                               ,server_cc::Stream::ptr stream) {
    auto rock_req = std::dynamic_pointer_cast<server_cc::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<server_cc::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<server_cc::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(server_cc::Message::ptr notify
                              ,server_cc::Stream::ptr stream) {
    auto rock_nty = std::dynamic_pointer_cast<server_cc::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<server_cc::RockStream>(stream);
...
void ModuleManager::init() {
    ...
    server_cc::FSUtil::ListAllFile(files, path, ".so");
