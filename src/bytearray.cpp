...
bool ByteArray::isLittleEndian() {
    return m_endian == SERVER_CC_BIG_ENDIAN;
}
void ByteArray::setIsLittleEndian(bool val) {
    if(val){
        m_endian = SERVER_CC_LITTLE_ENDIAN;
    }else{
        m_endian = SERVER_CC_BIG_ENDIAN;
    }
}
...
void ByteArray::writeFint16(int16_t value){
    if (m_endian != SERVER_CC_BYTE_ORDER){
...
void ByteArray::write(const void* buf,size_t size){
    ...
    while(size > 0){
        if(ncap >= size){
            ...
            if(m_cur->size == (npos + size)){//当前节点写满
                m_cur = m_cur->next;
            }
            m_position += size;
            bpos += size;
...
bool ByteArray::writeToFile(const std::string& name) const {
    std::ofstream ofs;//? ofstream 与 ifstream 待了解
    ofs.open(name, std::ios::trunc | std::ios::binary);
    if(!ofs) {
        SERVER_CC_LOG_ERROR(g_logger) << "writeToFile name=" << name
...
bool ByteArray::readFromFile(const std::string& name) {
    ...
    if(!ifs) {
        SERVER_CC_LOG_ERROR(g_logger) << "readFromFile name=" << name
            << " error, errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }

    std::shared_ptr<char> buff(new char[m_baseSize], [](char* ptr) { delete[] ptr;});//? shared_ptr自定义析构函数
    while(!ifs.eof()) {
