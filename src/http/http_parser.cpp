...
static server_cc::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    server_cc::Config::Lookup("http.response.buffer_size"
                ,(uint64_t)(4 * 1024), "http response buffer size");

static server_cc::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
    server_cc::Config::Lookup("http.response.max_body_size"
...
void on_request_method(void *data, const char *at, size_t length) {
    ...
    if(m == HttpMethod::INVALID_METHOD) {
        SERVER_CC_LOG_WARN(g_logger) << "invalid http request method: "
...
void on_request_fragment(void *data, const char *at, size_t length) {
    //SERVER_CC_LOG_INFO(g_logger) << "on_request_fragment:" << std::string(at, length);
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
...
void on_request_version(void *data, const char *at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        SERVER_CC_LOG_WARN(g_logger) << "invalid http request version: "
            << std::string(at, length);
...
void on_request_http_field(void *data, const char *field, size_t flen
    ...
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
    if(flen == 0) {
        SERVER_CC_LOG_WARN(g_logger) << "invalid http request field length == 0";
...
HttpRequestParser::HttpRequestParser()
    :m_error(0) {
    m_data.reset(new server_cc::http::HttpRequest);
...
void on_response_version(void *data, const char *at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        SERVER_CC_LOG_WARN(g_logger) << "invalid http response version: "
            << std::string(at, length);
        parser->setError(1001);
        return;
    }
...
void on_response_http_field(void *data, const char *field, size_t flen
    ...
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
    if(flen == 0) {
        SERVER_CC_LOG_WARN(g_logger) << "invalid http response field length == 0";
        //parser->setError(1002);
        return;
    }
    parser->getData()->setHeader(std::string(field, flen)
...
HttpResponseParser::HttpResponseParser()
    :m_error(0) {
    m_data.reset(new server_cc::http::HttpResponse);
