/*
 * @Author: closing
 * @Date: 2023-05-30 09:03:26
 * @LastEditors: closing
 * @LastEditTime: 2023-06-19 23:44:44
 * @Description: 请填写简介
 */
...
HttpConnection::~HttpConnection(){
    SERVER_CC_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse(){
    ...
    std::string body;
    //TODO chunked后的http请求是怎样的
    if(client_parser.chunked) {
        int len = offset;//上一步剩余的还没被解析的长度，
        do {
            bool begin = true;
            do {
                if(!begin || len == 0) {
                ...
                data[len] = '\0';//http11_parser要求字符串结尾必须是'\0'
            ...
            //len -= 2;
            //2 表示 /r/n 因为上面的parser->execute(data, len, true)会把/r/n解析掉
            SERVER_CC_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
    ...
    if(!body.empty()) {
        ...
        // SERVER_CC_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
        //     << " size=" << body.size();
...
int HttpConnection::sendRequest(HttpRequest::ptr req){
    ...
    SERVER_CC_LOG_DEBUG(g_logger) << "sendRequest:" << std::endl
        << data;
    return writeFixSize(data.c_str(),data.size());//? 函数嵌套占用事件会更长吗
...
HttpResult::ptr HttpConnection::DoRequest(HttpMethod method,
    ...
    if(!uri){
        SERVER_CC_LOG_ERROR(g_logger) << "invalid url: " << url;
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri,
    ...
    Uri::ptr tmp_uri = Uri::Create(uri);
    if(!tmp_uri) {
        SERVER_CC_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
    return std::make_shared<HttpConnectionPool>(tmp_uri->getHost()
            , vhost, tmp_uri->getPort(), tmp_uri->getScheme() == "https"
...
HttpConnection::ptr HttpConnectionPool::getConnection(){
    ...
    if(!ptr){
        ...
        if(!addr) {
            SERVER_CC_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            SERVER_CC_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            SERVER_CC_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= server_cc::GetCurrentMS())
