...
Socket::ptr Socket::CreateTCP(server_cc::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(server_cc::Address::ptr address) {
...
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    ...
    if(rt) {
        SERVER_CC_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
...
bool Socket::setOption(int level, int option, const void* result, socklen_t len) {
    if(setsockopt(m_sock, level, option, result, (socklen_t)len)) {
        SERVER_CC_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
            << " level=" << level << " option=" << option
            << " errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if(::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        SERVER_CC_LOG_ERROR(g_logger) << "bind error errrno=" << errno
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if(!m_remoteAddress) {
        SERVER_CC_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(timeout_ms == (uint64_t)-1) {
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            SERVER_CC_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, server_cc::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, server_cc::IOManager::WRITE);
}
